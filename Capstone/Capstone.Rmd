...
## Tạo bảng thống kê năm 2015
```{r, echo=FALSE}
#Lọc dữ liệu 2015
df %>% filter(year == 2015) %>% tbl_summary(
  by = status,
...
plot_country <- function(country_name, indicator){
  plot_data_func <- df %>% filter(country == country_name, year>=2000 & year<=2015 )
