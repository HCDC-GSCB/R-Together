...
df_plot2<- ggplot(df_plot, aes(x = coverage, y = inequality)) +
  geom_point(aes(size = size, color = color), alpha = 0.8) +
  geom_text(aes(label = country), hjust = -0.2, vjust = 0.2) +
  geom_vline(xintercept = 0, color = "#999999") + # bắt đầu từ giá trị 0 của cột x
  geom_hline(yintercept = 0, color = "#999999") + # bắt đầu từ giá trị 0 của cột y
  ...
  scale_size_continuous(
    ...
    range = c(0, 8),
    guide = guide_legend(order = 1) # thứ tự cú thích
  ) +
  scale_color_manual(
    values = cols,
    breaks = c("red", "green", "darkblue", "yellow", "blue", "purple"), #thứ tự hiện mãu sắc
    labels = c(
...
# thực hành 2
df_plot3 <- ggplot(df_plot, aes(x = coverage, y = inequality)) +
  geom_point(aes(size = size, color = color, alpha = 0.8)) +
  geom_text(aes(label = country), hjust = -0.2, vjust = 0.2) +
...
# Lưu hình ảnh
getwd()
ggsave("df_plot2.png", plot = df_plot2, width = 8, 
       height = 6, bg="white", dpi = 600)
